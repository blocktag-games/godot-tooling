...
| 2. Evaluate coverage and measurement quality | BP01, BP03, and BP04 closed. BP02 substantially closed (one oracle schema, all 16 oracles unified, F062 has a committed check; second-reviewer pass deliberately deferred, see implementation-plan.md). BP05 onward (remaining candidates, full tier-1 corpus) pending. | Shared correctness corpus, controlled performance study, profiling/observer experiments, and reproducible public report. |
...
BP01, BP03, and BP04 are closed. BP02 is substantially closed: an internal review found specific unmet completion evidence (no unified oracle schema, F062 untested by any committed script, no second-reviewer pass on any oracle) and all but the reviewer pass were fixed the same day -- see [implementation-plan.md](benchmarks/implementation-plan.md) for what's deliberately deferred and why (a second-reviewer pass on the 16 oracles is in progress via an independent review agent). BP04's remaining gap -- "one-command reproduction" wasn't literally true, since `compare.py` read committed artifacts rather than re-invoking `gd-tools test --coverage` -- is closed: `pilot/adapter-run/reproduce.sh <case>` restores the gitignored `addons/`, rebuilds the import cache, runs the real tool, and compares, verified by actually deleting `addons/`/`.godot/`/`.gd-tools/` and re-running from that state. One incorrect finding in BP04's original write-up was also corrected the same day it was found. See the [pilot environment pin](benchmarks/pilot-environment.md) for the exact Godot 4.7.1 binary/hash, gd-tools-cli 0.4.0 pin, and GUT v9.7.1 pin; [pilot/fixtures/](../pilot/fixtures/) for the 16-oracle tier-0 corpus, now sharing one oracle schema (`pilot/harness/oracle.py`) instead of four incompatible ones; [pilot/harness/](../pilot/harness/) for the evidence harness, now with isolated workspaces and structured outcome records; and the [BP04 adapter run](../pilot/adapter-run/README.md) for the first real `gd-tools test --coverage` run: a clean match on one fixture, plus three confirmed findings on another, the sharpest being that gd-tools' branch-coverage percentage can read 100% while a branch's body never executed (its `if_true` counter fires on every evaluation of the decision, not on the true outcome specifically). Keep GdUnit4 Coverage as an optional feature comparison with its exact patched binaries and public tracking limit recorded.
