...
The immediate next work is BP01–BP04: pin the pilot environment and tools, implement 12 small baseline/oracle cases, build the minimal evidence harness, and add one coverage adapter. BP01 is closed — see the [pilot environment pin](benchmarks/pilot-environment.md) for the exact Godot 4.7.1 binary/hash, gd-tools-cli 0.4.0 pin, and GUT v9.7.1 pin. Start by investigating gd-tools/GUT, then expand to Nano Coverage and godot-code-coverage with their supported runner/manual modes. Keep GdUnit4 Coverage as an optional feature comparison with its exact patched binaries and public tracking limit recorded.
