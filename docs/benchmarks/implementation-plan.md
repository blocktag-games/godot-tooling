...
Status on 19 September 2026: protocol and catalogs drafted; BP01 and BP03 are closed, BP02 is substantially closed with one deliberately-deferred item, BP04 remains **partial**. An internal review on this date found specific unmet completion evidence in BP02-BP04 and, separately, one incorrect finding in BP04's original write-up (since corrected). Most of the named gaps in BP02 and BP03 were closed the same day; see each row. See [pilot-environment.md](pilot-environment.md), [../../pilot/fixtures/](../../pilot/fixtures/), [../../pilot/harness/](../../pilot/harness/), and [../../pilot/adapter-run/](../../pilot/adapter-run/). Work packages below are ordered by dependency. Estimates are planning ranges for one active contributor and will be revised after the first runnable slice.
...
| BP04 | Add the first coverage adapter. | BP02, BP03 | **Partial as of 2026-09-19.** [pilot/adapter-run/](../../pilot/adapter-run/): real `gd-tools test --coverage` (gd-tools-cli 0.4.0 + GUT v9.7.1) run against F010 (clean match against its oracle, now loaded via `pilot/harness/oracle.py` rather than ad hoc field access) and F020. F020's original write-up misdiagnosed its branch-count finding as an "unresolved anomaly"; a same-day correction, backed by three independent runs (both branches, true-only, false-only) plus reading `coverage.gd`'s injection logic, confirmed the real finding: gd-tools' `if_true` branch counter fires on every evaluation of the decision, not on the true outcome specifically, so it can report 100% branch coverage while the true branch's body never executed. See `pilot/adapter-run/f020_if_both_outcomes/compare.py`. **Still not met:** "one-command reproduction" is not literally true from a clean checkout -- `compare.py` reads already-committed `plan.json`/`coverage.json` artifacts rather than re-invoking `gd-tools test --coverage` itself, and the vendored `addons/` directories it depends on are gitignored, not restored by any script. Deliberately not addressed in this pass; scoped as its own follow-up. | 2–4 days |
