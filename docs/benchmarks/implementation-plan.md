...
Status on 19 September 2026: protocol and catalogs drafted; BP01, BP03, and BP04 are closed, BP02 is substantially closed with one deliberately-deferred item (a genuine second-reviewer pass on the 16 oracles, currently in progress via an independent review agent). An internal review on this date found specific unmet completion evidence in BP02-BP04 and, separately, one incorrect finding in BP04's original write-up (since corrected). All named gaps were closed the same day except BP02's deferred item; see each row. See [pilot-environment.md](pilot-environment.md), [../../pilot/fixtures/](../../pilot/fixtures/), [../../pilot/harness/](../../pilot/harness/), and [../../pilot/adapter-run/](../../pilot/adapter-run/). Work packages below are ordered by dependency. Estimates are planning ranges for one active contributor and will be revised after the first runnable slice.
...
| BP02 | Implement the first independent fixtures and oracles. | BP01 | **Substantially closed 2026-09-19; one item deliberately deferred.** All 16 tier-0 oracles now share one schema (`pilot/harness/oracle.py`'s `load_oracle`/`obligations_for_input`/`union_obligations`), replacing four incompatible `expected_hit` shapes; validated against real BP04 evidence, including a genuinely new mechanized result (F020's `union_obligations` reproduces the "line 4 has no trackable point" finding that previously needed hand-written prose). F062 now has a committed, runnable check (`cases/f062_injected_instrumentation_failure/check.py`) instead of an ad hoc inline test. F008/F064/F071 still stand as real, reproducible findings against the pinned tools. **Deferred, not fixed:** no oracle has a second reviewer in the literal sense of a distinct person re-deriving each oracle from its fixture (`"reviewer": null` remains on all 16 files) -- the project owner has directed that the 2026-09-19 external frontier-model review (which examined F040/F091/F008/F071/BP04's F020 finding in detail and found the branch-coverage misdiagnosis) satisfies this project's review-gate requirement for now. That review did not re-derive every oracle line-by-line, so the `reviewer` field is left honestly `null` rather than stamped with a claim broader than what was actually checked. | 2–4 days |
