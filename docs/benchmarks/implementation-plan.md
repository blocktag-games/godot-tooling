...
Status on 19 September 2026: protocol and catalogs drafted; BP01 is closed. BP02, BP03, and BP04 are **partial** -- an internal review on this date found specific unmet completion evidence and, separately, one incorrect finding in BP04's original write-up (since corrected; see BP04's row). See [pilot-environment.md](pilot-environment.md), [../../pilot/fixtures/](../../pilot/fixtures/), [../../pilot/harness/](../../pilot/harness/), and [../../pilot/adapter-run/](../../pilot/adapter-run/). Work packages below are ordered by dependency. Estimates are planning ranges for one active contributor and will be revised after the first runnable slice.
...
| BP02 | Implement the first independent fixtures and oracles. | BP01 | **Partial as of 2026-09-19.** All 15 tier-0 cases built and pass on the pinned engine; F008/F064/F071 produced real, reproducible findings against the actual pinned tools. Not met: no oracle has a second reviewer (`"reviewer": null` on all 16 files, vs. the stated "reviewed obligations"); F062 has no committed check script (its fault-injection test was only run inline, ad hoc); oracle `expected_hit` is expressed in four incompatible shapes across cases (flat bool, per-input dict, `branch_outcomes` object, `not_a_separate_obligation` sentinel) with no shared schema or loader, so only F010 can be compared mechanically via `pilot/harness/comparator.py` -- every other case needs bespoke translation code, as F020 required in BP04. This last point should be fixed (one oracle schema + loader) before BP06 multiplies it across ~70 more cases. | 2–4 days |
| BP03 | Implement minimal orchestration and evidence records. | BP01 | **Partial as of 2026-09-19.** [pilot/harness/](../../pilot/harness/)'s comparator, runner, and fault-injection modules exist and 8/8 self-tests pass. Not met: no isolated workspace mechanism exists (fixtures/adapter-run cases are run in place, not in per-run isolated copies); no structured outcome record format exists (results are ad hoc printed text, not a schema); two of the eight self-tests don't exercise the harness -- the null-adapter test compares two hardcoded identical strings, and the truncated-JSON test calls Python's `json` module directly rather than any harness code; `comparator.py`'s `compare()` never reads or checks an `Obligation`'s own `sha256` field, so nothing actually enforces the oracle-to-source binding it's supposed to. | 2–4 days |
| BP04 | Add the first coverage adapter. | BP02, BP03 | **Partial as of 2026-09-19.** [pilot/adapter-run/](../../pilot/adapter-run/): real `gd-tools test --coverage` (gd-tools-cli 0.4.0 + GUT v9.7.1) run against F010 (clean match against its oracle) and F020. F020's original write-up misdiagnosed its branch-count finding as an "unresolved anomaly"; a same-day correction, backed by three independent runs (both branches, true-only, false-only) plus reading `coverage.gd`'s injection logic, confirmed the real finding: gd-tools' `if_true` branch counter fires on every evaluation of the decision, not on the true outcome specifically, so it can report 100% branch coverage while the true branch's body never executed. See `pilot/adapter-run/f020_if_both_outcomes/compare.py`. Not met: "one-command reproduction" is not literally true from a clean checkout -- `compare.py` reads already-committed `plan.json`/`coverage.json` artifacts rather than re-invoking `gd-tools test --coverage` itself, and the vendored `addons/` directories it depends on are gitignored, not restored by any script. | 2–4 days |
