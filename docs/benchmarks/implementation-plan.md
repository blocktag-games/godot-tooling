...
Status on 19 September 2026: protocol and catalogs drafted; BP01 through BP04 are closed (see [pilot-environment.md](pilot-environment.md), [../../pilot/fixtures/](../../pilot/fixtures/), [../../pilot/harness/](../../pilot/harness/), and [../../pilot/adapter-run/](../../pilot/adapter-run/)). BP05 is next. Work packages below are ordered by dependency. Estimates are planning ranges for one active contributor and will be revised after the first runnable slice.
...
| BP04 | Add the first coverage adapter. | BP02, BP03 | **Closed 2026-09-19.** [pilot/adapter-run/](../../pilot/adapter-run/): real `gd-tools test --coverage` (gd-tools-cli 0.4.0 + GUT v9.7.1) run against F010 (clean match against its oracle) and F020 (two confirmed findings -- an uninitialized `var` has no trackable point at all; branch line attribution differs from the oracle's convention -- plus one open, unresolved branch-count anomaly across multi-test runs). One-command reproduction via each case's `compare.py`. | 2–4 days |
