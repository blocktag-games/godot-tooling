...
| [BP05 candidates](pilot/candidates/) | godot-code-coverage confirmed incompatible with Godot 4.7.1; Nano Coverage builds and loads but its headless API crashes; GdUnit4 confirmed working. Each candidate's `PIN.md` has the full findings. |
...
The benchmark plan specifies 88 correctness/integration cases, 20 workloads, and 28 experiment families; most remain planned catalogs, not implemented tests. The 16-oracle tier-0 vertical slice and the first gd-tools+GUT adapter run are implemented under `pilot/` (see the pilot environment pin and BP04/BP05 rows above), with three confirmed findings against gd-tools plus real, current compatibility findings against Nano Coverage and godot-code-coverage. An internal review found specific gaps against each package's stated completion evidence; BP01 through BP05 are now all closed, including a genuine second-reviewer pass (by an independent review agent) on all 16 oracles, which itself found and fixed four further real issues -- see `docs/benchmarks/implementation-plan.md` for the exact, current state of each package. Compare gd-tools, Nano Coverage, and godot-code-coverage, with GdUnit4 Coverage as a separately scoped feature comparator.
