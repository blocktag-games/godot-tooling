...
The benchmark plan specifies 88 correctness/integration cases, 20 workloads, and 28 experiment families; most remain planned catalogs, not implemented tests. The 15-case tier-0 vertical slice and the first gd-tools+GUT adapter run are implemented under `pilot/` (see the pilot environment pin and BP04 adapter run rows above), with three confirmed findings against the real, pinned tools -- but an internal review found specific gaps against each package's stated completion evidence (oracle review, harness workspace isolation, literal one-command reproduction), so BP02–BP04 are tracked as partial, not closed; see `docs/benchmarks/implementation-plan.md` for the exact list. Compare gd-tools, Nano Coverage, and godot-code-coverage, with GdUnit4 Coverage as a separately scoped feature comparator.
